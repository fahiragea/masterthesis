...
policy_colors = {
    "Best Egypt Irr.": "blue",
    "Best Egypt HAD": "green",
    "Best Sudan Irr.": "purple",
    "Best Ethiopia Hydropower": "chocolate",
    "Compromise: Percentile": "yellow",
    "Compromise: Absolute": "red"
}
...
def plot_robustness_parallel(outcomes, metric_type):
    ...
    units = ["Score", "Score", "Score", "Score"]

    # Generate the parallel plot
    fig = plt.figure()
    ax1 = fig.add_subplot(111)

    parallel_coordinates(
        outcomes.reset_index(),
        "policy",
        color=[policy_colors[policy] for policy in outcomes.index],
        linewidth=1,
        alpha=0.7
    )
    ...
    labels = list(policy_colors.keys())
    ...
    ax1.set_xticks(np.arange(len(names_display)))
    ax1.set_xticklabels(
        [
            f"{names_display[i]}\n[{units[i]}]"
            for i in range(len(names_display))
        ],
        fontsize=10
    )

    ax2 = ax1.twiny()
    ax2.set_xticks(np.arange(len(names_display)))

    min_values = outcomes.min().values
    max_values = outcomes.max().values

    ax1.set_xticklabels([f"{min_values[i]:.2f}" for i in range(len(names_display))], fontsize=10)
    ax2.set_xticklabels([f"{max_values[i]:.2f}" for i in range(len(names_display))], fontsize=10)
    ...
    print("Finished plotting")  # Debugging print

def plot_robustness_score(weighted_sum_scores):
    # Prepare the display names
    names_display = weighted_sum_scores.columns.tolist()
    units = ["Agg. Ranking"] * len(names_display)

    # Normalize the data for plotting such that lower values are better
    normalized_data = (weighted_sum_scores.max() - weighted_sum_scores) / (weighted_sum_scores.max() - weighted_sum_scores.min())
    normalized_data['policy'] = weighted_sum_scores.index
    ...
    parallel_coordinates(
        normalized_data,
        "policy",
        color=[policy_colors[policy] for policy in weighted_sum_scores.index],
        linewidth=2,  # Increase the linewidth for better visibility
        alpha=0.7
    )
    ...
    min_values = weighted_sum_scores.min().values
    max_values = weighted_sum_scores.max().values
    ...
    ax2.set_xticklabels(
        ...
        fontsize=14  # Increase x-axis top label font size
    )
    ...
    plt.text(
        ...
        0.5,
        "Direction of Preference $\\rightarrow$",
        {"color": "#636363", "fontsize": 18},  # Increase font size for the direction label
        horizontalalignment="left",
        verticalalignment="center",
        rotation=90,
        clip_on=False,
...
def plot_robustness_parallel(outcomes, metric_type):
    ...
    # Normalize the data for plotting such that lower values are better
    normalized_data = (outcomes.max() - outcomes) / (outcomes.max() - outcomes.min())
    normalized_data['policy'] = outcomes.index
...
def plot_robustness_parallel2(outcomes, metric_type, directions):
    # Prepare the display names
    names_display = [
        "Egypt Irr. Deficit",
        "Egypt Low HAD",
    ...
    # Normalize the data for plotting based on the direction
    normalized_data = outcomes.copy()
    for i, col in enumerate(outcomes.columns):
        if directions[i] == "min":
            normalized_data[col] = (outcomes[col] - outcomes[col].min()) / (outcomes[col].max() - outcomes[col].min())
        else:
            normalized_data[col] = (outcomes[col].max() - outcomes[col]) / (outcomes[col].max() - outcomes[col].min())
    ...
    # Determine the direction of preference text
    direction_text = "Direction of Preference $\\rightarrow$"
